...
These questions touch on core architectural decisions that any declarative data stack must address. In exploring my initial proof-of-concept for the [declarative-data-stack-engine](https://github.com/ssp-data/declarative-data-stack-engine/), I discovered that choosing between a template engine versus a starter kit or between custom infrastructure versus existing orchestration tools influences how we build and maintain data stacks at scale.
...
A quick reminder: Why do we use a declarative data stack as introduced in [Part 1](https://www.ssp.sh/blog/rise-of-declarative-data-stack/)? It allows us to automate the entire data stack, focusing on the business logic, not technical implementation. Tools that lack these declarative code paths are difficult to automate, because code generation is almost impossible (imagine trying to auto-generate Tableau dashboards or Informatica pipelines). By applying software engineering best practices we can build complete “data stack compilers” that run tests *before* being put into production, and build versioned, maintainable, and reproducible data stacks.
...
1. What if we could **generate** the declarative code for an entire data pipeline with just a few lines of config?
2. How can we truly **separate** our business logic from the technical implementation?
3. Could we **test** the entire data stack locally before pushing it to production?
4. Could we **annotate** our table types – staging, slowly-changing dimension tables, fact tables – in a single config/annotation?
...
6. What if we could declare every piece in our data stack using a single consistent language, such as **SQL**?
...
We’ll start with the templated data stack. Below we see the configuration file 1 that defines the full data stack from ingestion to serving. Not only can this config be consumed by the declarative data stack, but even external services (e.g., GDPR checks or integrations with SAP, etc.) could potentially read and integrate that too (full code in [1-simple-stack.py](https://github.com/ssp-data/declarative-data-stack-engine/blob/main/simple-example/1-simple-stack.py)).
...
You see, the configuration is simple to understand, and the business logic, like transformation, metrics, or dashboards, is in one place. We can even use abstractions such as referencing the logical name `sales_daily` in our dashboard that’s generated in transformation.
...
Comparing it to a simpler, but imperative data stack (full code in [1-simple-stack.py](https://github.com/ssp-data/declarative-data-stack-engine/blob/main/simple-example/1-simple-stack.py)), with no configuration file, it looks like this:
...
The logs are the same, but the implementation differs. Notice in this example that the Python script executes the declarative data stack, not only generating the [stack_config.yaml](https://github.com/ssp-data/declarative-data-stack-engine/blob/main/simple-example/config/stack_config.yaml) and [dashboards yaml](https://github.com/ssp-data/declarative-data-stack-engine/tree/main/simple-example/dashboards).
...
It’s essential to distinguish between declarative systems and merely a declarative wrapper. For example, a tool like [dbt](https://github.com/dbt-labs/dbt-core) is a declarative wrapper around SQL, not a truly declarative interface. By adding another layer on top, we introduce yet another interface.
...
Can I get a better separation of concerns with the advanced out-of-the-box features? For example, Dagster already supports many features, such as separating business logic in [asset](https://docs.dagster.io/concepts/assets/software-defined-assets) [functions](https://dagster.io/glossary/software-defined-assets), executing infrastructure with various compute integrations, and having technical implementation abstracted elegantly through resources.
...
For example, the asset for the ingestion (full code in [**4-dagster-stack.py**](https://github.com/ssp-data/declarative-data-stack-engine/blob/main/simple-example/4-dagster-stack.py)):
...
It is interesting that most of the code will fall back to its description as most of the boilerplate and technical implementation is handled by Dagster. And anything we want to add, we should pack into reusable, technical [resources](https://docs.dagster.io/concepts/resources) that, for example, Dagster offers.
...
,Â
 andÂ
 to manage more of the data stack
-Â
...
- Find more on [Closed-Source Data Platforms](https://www.ssp.sh/brain/closed-source-data-platforms) and a fantastic read on composable data stacks on [a new frontier](https://voltrondata.com/codex/a-new-frontier#composable-data-systems) by Voltron Data.
...
- [**BoilingData**](https://github.com/boilingdata/boiling-insights)**:** A local-first data processing native application designed for rapid data pipeline development. Enables data engineers to build and test pipelines quickly using tools like DuckDB, dbt, and dlt.
- [**HelloDATA**](https://github.com/kanton-bern/hellodata-be): An enterprise data platform built on open-source tools based on the modern data stack. It uses state-of-the-art tools such as dbt for data modeling with SQL and Airflow to run and orchestrate tasks, Superset to visualize the BI dashboards, and JupyterHub for data science tasks. It includes multi-tenancy, full authentication, and authorization, which are handled with a single web portal.
...
- [**SQLMesh**](https://github.com/TobikoData/sqlmesh): An efficient data transformation and modeling framework that has compiler capabilities built-in with [SQLGlot](https://github.com/tobymao/sqlglot), a Python SQL parser and a transpiler.
- [**GitHub Actions**](https://github.com/features/actions): This is the [simplest version](https://bsky.app/profile/mehdio.com/post/3lcndhb4tu22l) for building a declarative data stack. A  `deploy.yaml` script could be a simple DDS config. GitHub also has an engine that converts and runs on Docker-runners. So, in a way, it’s another engine implementation, and maybe we could take some configs based on that?
...
 would be a good integration to parse SQL without running.  Same as SDF integration with Datafusion.
Existing Templating
...
Openness is also key to creating an environment where **trust** and **visibility** flourish, mainly through open source. All the code is open, with **no lock-in** or proprietary format/engine. If you want to go back, it started from the very beginning. Open source had its [roots](https://opensource.org/history) shortly before the announcement of Netscape, and it got mainstream with the recognition of Linux. Today, open source is the key to openness.


#### Examples of Open Standards at Work


Also, people can work together. It’s a little with the data lakehouse we’ve seen. This is built on top of open source. There is less integration than a traditional data warehouse, but it is easier to exchange a tool, access your data, and be more open. And, you can still add proprietary functions and tools on top, such as Databricks has done with [Photon Engine](https://www.databricks.com/product/photon) and some machine learning capabilities. Snowflake did similar things when integrating Apache Iceberg, [Snowplow](https://github.com/snowplow/snowplow), and another open-source tool to open the platform.
...
The beauty of open-source is that the more you share, the more feedback, contributions, and, most importantly, trust a product receives. It allows fast iteration cycles with multiple participants who can contribute, and everything is version-controlled and transparent—which makes it best of breed forÂ
**security**
 reasons to find and fix them fast. Usually, open sourcing improves a product, as contributions from the community cannot be compared to those made by a small team of in-house engineers.
