...
![Data Modeling â The Unsung Hero of Data Engineering: An Introduction to Data Modeling (Part 1)](https://www.ssp.sh/blog/data-modeling-for-data-engineering-introduction/images/data-modeling-introduction-feature.jpg)

Contents

Amidst the excitement and hype surrounding artificial intelligence, the significance of data engineering and its critical foundationâdata modelingâcan often be overlooked. This article is the first in a three-part series that will shine a spotlight on the fascinating world of data modeling, delving into its crucial importance within the broader context of data engineering. We will explore the history of data modeling, pioneered by visionaries like Kimball and Inmon, and discuss the necessity for a comprehensive understanding of data architecture in today’s data-driven world.
...
An essential part of data modeling is theÂ **business logic**, which gets integrated inside the data flow either as SQL/Python code, some drag-and-drop ETL tool, or inside a modernÂ [Business Rule Engine](https://en.wikipedia.org/wiki/Business_rules_engine).


Most data projects initially ignore data modeling because of time constraints, fast-paced development, or sometimes lack of knowledge. Instead of investing time into sketching out how the data should flow from its source, the organizationâs requirements, to how the end user wants to analyze its data, projects get built quickly and with a “dirty”Â [MVP](https://en.wikipedia.org/wiki/Minimum_viable_product).
...
Data modeling goes hand in hand with clear goals. It integrates technical and data requirements as well as business and performance requirements. With these, aligned within the company, itâs much easier to understand the business from a data perspective and be successful in the long run.
...
Data modeling can beÂ **applied at different levels**Â and contains more than just modeling. Besides the conceptual, logical, and physical data model, you can model your source OLTP database, warehouse, BI tool, and ML features. More importantly, we discuss further in this article the overall data modeling across the organization.


In the end, each layer of theÂ [Data Engineering Lifecycle](https://glossary.airbyte.com/term/data-engineering-lifecycle/)Â can be modeled. To name just a few:

1. Generation or source database: Model the entities of your applicationâs source database, and normalize tables to itsÂ [Third normal form (3NF)](https://en.wikipedia.org/wiki/Third_normal_form). Choose the best features of the database of choice.
...
5. BI tools and reporting (Presentation level): Designing the data structures, aggregations, and calculations used in reporting and visualization tools. Building the presentation layer may involve creating semantic layers, such as data cubes, that simplify business usersâ access to the underlying data.
...
Considering these different levels, the key is toÂ **model the business requirements**Â end to end. Sometimes these are modeled insideÂ [Business Rules](https://en.wikipedia.org/wiki/Business_rule)Â where calculations and processes organization are stored. By integrating business logic into data modeling, organizations can ensure that their data architecture is aligned with their objectives and supports effective decision-making.
...
2. Emergence ofÂ **relational databases (1980s)**: The relational model, introduced byÂ [Edgar F. Codd](https://en.wikipedia.org/wiki/Edgar_F._Codd), revolutionized data modeling and management. Relational databases enabled the development of theÂ [Entity Relationship Diagram (ERD)](https://en.wikipedia.org/wiki/Entity%E2%80%93relationship_model)Â model, which became the dominant approach to designing and implementing databases for transactional systems.
3. Birth ofÂ **data warehousing and BI (1990s)**: BI and data warehousing gained prominence during this period. Ralph Kimball introduced dimensional modeling, becoming the standard for data warehouse design. Bill Inmon proposed another approach, the top-down approach, which focused on creating an enterprise-wide, normalized data model. Kimballâs and Inmonâs approaches addressed the challenges of analyzing and reporting data for decision-making.
4. Expansion ofÂ **data sources and big data (2000s)**: As organizations started collecting and analyzing data from various sources, including the web and social media, the volume, variety, and velocity of data increased significantly. This data growth led to the emergence of big data technologies and platforms likeÂ [Hadoop](https://glossary.airbyte.com/term/apache-hadoop/), NoSQL databases, and data lakes. Data modeling in this context had to adapt to the changing landscape, incorporating new data types and storage systems.
5. ModernÂ **data engineering (2010s - present)**: With the decline for computation the rise of cloud computing, advanced analytics, and machine learning, data engineering has evolved to handle increasingly complex and diverse data ecosystems. Data modeling in the modern data engineering landscape needs to accommodate various data sources, formats, and storage technologies, such as cloud-based data warehouses, data lakes, and real-time streaming data.
...
In theÂ **context of modern data engineering**, data modeling focuses on creating flexible, scalable, and efficient data models that cater to diverse analytical and processing requirements. New techniques like data vault modeling, data mesh, and others have emerged, and weâll discuss the differences later in [part two](https://www.ssp.sh/blog/data-modeling-for-data-engineering-approaches-techniques/).
...
After the boom andÂ [explosion](https://mad.firstmarkcap.com/)Â of the data landscape,Â **people have realized that a solid data architecture**Â and choice of tools matter a lot regarding theÂ **success of a data-driven culture**Â and the cost of using data. Therefore making it relevant for todayâs data challenges. With more requirements and near real-time analytics on the rise, we need faster ways to analyze data. The organizations need help withÂ [Data Governance](https://glossary.airbyte.com/term/data-governance/)Â as more data we need to ensure better data quality, consistency, and compliance with regulations such asÂ [GDPR](https://gdpr.eu/)Â andÂ [CCPA](https://oag.ca.gov/privacy/ccpa/).
...
With all these challenges, focusing on the critical part is tricky. Thatâs where data modeling comes in,Â **zooming in on the business outcome**Â and aligning data modeling around it. Data modeling helps with seeing the end-to-end data flow from a higher level. Instead of thinking in your small garden ahead, youÂ **zoom out and see the whole field**. The outcome is a clear strategy, solid data architecture, and more straightforward solutions.


Thatâs why data modeling and, with it, dimensional modeling is getting popular again. People recommend it on social media, giving talks on “[Is Kimball still relevant](https://coalesce.getdbt.com/blog/babies-and-bathwater-is-kimball-still-relevant),” and even writing books about it, e.g.Â [Joe Reis on “modern” data modeling practices](https://www.linkedin.com/posts/josephreis_im-writing-a-new-book-on-modern-data-modeling-activity-6991977435017728000-JqyE?utm_source=share&utm_medium=member_desktop), orÂ [Serge Gershkovich writing about Bringing back Data Modeling](https://www.linkedin.com/posts/serge-cloud-connected_bringbackdatamodeling-activity-6993236783610114048-I4A9?utm_source=share&utm_medium=member_desktop).


Letâs next look at how data complexity can be reduced with data modeling and help us see the big picture.


## The Complexity of Data


Data and its ecosystem is rapidly getting more complex. Especially when working with dataâdata is always outdated, dependent on third-party tools or services, and you have multiple sources and even systems. All of this makes data very hard to work with.


Besides these apparent difficulties, modeling data is even more complex. It would be best to think about storing data, transforming, aggregating, the right granularity, rolling them up, and much more. These are things weâll cover in this chapter.


### Big-O Notation, Why It Matters


Moving data can grow exponentially fast if you have no focus on modeling your data well. If you do not know about that, you are probably not delivering on good performant code. Itâs close to programming with theÂ [Big-O](https://glossary.airbyte.com/term/big-o-notation/)Â notation. In essence, it defines the complexity of a function in a constant-time O(1), linear-time O(N), or quadratic-time O(N^2).


Suppose you donât take care of the times of copying a slightly different version of your dataset, essentially how the data flows through your organization. In that case, a lot of redundancies and high costs can result.
...
[Granularity](https://glossary.airbyte.com/term/granularity/)Â is related and essential to think about. The grain of your data defines with which details you store, adding to the duplication if you are not careful. For example, if you want to have one data aggregated by countries and another one with full details on each city, time, and user, with modeling, you can store it once and aggregate it on top.


There are techniques to help against duplicating calledÂ [roll up](https://glossary.airbyte.com/term/rollup/). Roll-ups drastically reduce the amount of data to store, in case you donât need these details. It keeps aggregates for faster query times to avoid duplicating all rows. For example, Druid and other databases have such featuresÂ [built-in](https://druid.apache.org/docs/latest/tutorials/tutorial-rollup.html).
...
After introducing data modeling, letâs look at key concepts of data modeling that apply to all data modeling techniques before we dive into each of the different data modeling techniques.


### Metrics/KPIs


[Metrics](https://glossary.airbyte.com/term/metric/),Â [KPI](https://glossary.airbyte.com/term/key-performance-indicator-kpi/)Â andÂ [(calculated) measure](https://glossary.airbyte.com/term/metric/)Â are terms that serve as the building blocks for how business performance is both measured and defined, as knowledge of how to define an organizationâs KPIs.


It is fundamental to have a common understanding of them. One step ahead, metrics are typically shown in business reports and dashboards with direct access to the entire organization. Metrics relate to data modeling as a way ofÂ **defining goals**. You must define them to know what you build the data stack for. With metrics agreed upon across the organization, you create a data model leading you there.


### Normalization: Normalized vs. Denormalized


[Normalization](https://glossary.airbyte.com/term/database-normalization), created by Edgar F. Codd, is a fundamental concept in relational data modeling aimed at minimizing redundancy and ensuring data consistency by avoiding multiple instances of stored data. A redundancy-free relational data model is often referred to as a normalized data model or as a data model inÂ [Third normal form (3NF)](https://en.wikipedia.org/wiki/Third_normal_form). In contrast, OLAP cubes and analytics solutions generally use dimensional modeling with dimensions and facts, which is optimized for fast query latency.


In practice, creating relational data models usually involves implicitly designing a model in the 3rd normal form without going through individual normalization steps in detail. Data can be categorized intoÂ **master data**Â andÂ **transactional data**, where master data describe business objects with properties that can change over time, and transactional data represents specific business events tied to the time of the event.


One well-known technique that emerged with “big data” is the One Big Table (OBT) approach, which creates a wideÂ [denormalized](https://glossary.airbyte.com/term/database-normalization/#denormalization)Â table containing all dimensions within the required granularity. Storing data in OBTs simplifies queries by eliminating the need for joins and offers faster response times, especially when usingÂ [columnar](https://glossary.airbyte.com/term/data-processing-techniques/#columnar-storage)Â storage formats. However, managing OBTs can be challenging, as updating or backfilling dimensions requires modifying many rows.Â [Data Lake Table Formats](https://glossary.airbyte.com/term/data-lake-table-format/)Â have made this process more manageable.
...
The most common approach is theÂ [Slowly Changing Dimension (Type 2)](https://glossary.airbyte.com/term/slowly-changing-dimension-scd). Â A Slowly Changing Dimension (SCD) isÂ **a dimension that stores and manages current and historical data over time in a data warehouse**. It is considered and implemented as one of the most critical ETL tasks in tracking the history of dimension records.


Other approaches are introducing snapshotting of everything. With the new data lake table formats, you have the capabilities ofÂ [Time Travel](https://glossary.airbyte.com/term/time-travel/), which essentially lets you go back and see each change that has been done. Be careful, though. The consequence is storing all your data multiple times and influencing how you query the data. You should add a timestamp like theÂ *as of*Â date to your query. You jump back in time and query the data as the end of last year if you snapshotted the state of this date. Another technique isÂ [Bitemporal Modeling](https://en.wikipedia.org/wiki/Bitemporal_modeling); more on that inÂ [part two](https://www.ssp.sh/data-modeling-for-data-engineering-approaches-techniques/).
...
InÂ [part two](https://www.ssp.sh/blog/data-modeling-for-data-engineering-approaches-techniques/), we will discuss differentÂ **data modeling techniques**, such as dimensional modeling, data vault modeling, and data mesh, among others. We will also explore variousÂ **data modeling approaches**Â that help organizations build scalable and maintainable data systems. The second part of the series will provide insights into choosing the right modeling technique for your organization, considering your unique requirements, challenges, and goals.


In [part three](https://www.ssp.sh/blog/data-modeling-for-data-engineering-architecture-pattern-tools-future/), we will delve intoÂ **data architecture patterns, tools, and the future**Â of data modeling. Stay tuned as we explore the fascinating world of data modeling within data engineering and its impact on the future of data-driven decision-making.
