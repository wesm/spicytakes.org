...
It always starts with a template. With `cmd+t` on Mac, I choose a Template. My default is `ð³ Permanent Note Template`, which contains the following content:
...
- `ð¬` Start any note, idea, something I read, or anything that comes to mind. Just some fleeting notes. This can also be deleted after a while
- `ð/ð»` I worked on it a bit. I added many fleeting notes, brainstormed, elaborated a bit, and made some references.
- `ð/ð` literature notes written and not ready for the [Permanent Notes](http://ssp.sh/brain/permanent-notes) / Evergreen Notes. Still, [Literature Notes](http://ssp.sh/brain/literature-notes) are formulated in whole sentences and have already worked, or I’m just happy with the content.
- `ð/ð³` Evergreen / Permanent Notes. These long-running notes will end up in [Zettelkasten](http://ssp.sh/brain/zettelkasten) core with my own words. Here, I separated the literature notes into different ideas to follow the zettelkasten principle and link them together.
...
This is what the `ð Book Template` looks like:
