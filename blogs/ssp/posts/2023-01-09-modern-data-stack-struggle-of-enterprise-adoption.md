...
In part I, [The Open Data Stack Distilled into Four Core Tools](https://www.sspaeti.com/blog/open-data-stack-core-tools/), we discussed how to quickly set up a data stack, tackling end-to-end data analytics challenges. As a manager or developer working with data at a mid- to large-sized enterprise, you might ask why aren’t we using any of these tools.Â


In this article, we dive into what mid-to-large-sized companies are using instead, the struggle of setting up a [Modern Data Stack (MDS)](https://glossary.airbyte.com/term/modern-data-stack/) for an enterprise size, and the opportunities of a free-of-charge and open-source MDS. We will also highlight its downside: the added [complexity](https://roundup.getdbt.com/p/complexity-the-new-analytics-frontier) of using dedicated tools along each phase of the [Data Engineering Lifecycle](https://glossary.airbyte.com/term/data-engineering-lifecycle).


## The Struggle of Building the Enterprise Data Stack


No matter what, building an enterprise-sized data stack is no piece of cake. Usually, in larger companies, everything is more complex. You have more people (possibly independent teams working with the same data), the whole setup is vastly more intertwined, and the code and programming language are enormous across the organization. There’s a lot of management with different opinions and strategies, and data engineering tools are [changing faster](https://mattturck.com/data2021/) than any other domain.


Usually, the struggle starts with a management strategy to be a **more data-driven** and self-serving organization. The struggle expands around eliminating data silos (Excel is your enemy), creating more transparency, faster decision cycles that are based on facts, and empowering people from the business to be more data savvy.

Top-Level Management Support
No matter which direction you go, it’s more important than ever to have top-level management support for your data strategy, executed from the bottom up—use-case driven, starting with one department at a time.
...
- go all in with a service from one of the big [cloud providers](https://glossary.airbyte.com/term/cloud-provider)
- [‍](https://glossary.airbyte.com/term/cloud-provider)Hire a consultancy firm that knows the latest trends—and can take responsibility if things are not working out ð


### Technology Adoption Lifecycle: The Chasm


If we look back at the [Chasm](https://en.wikipedia.org/wiki/Crossing_the_Chasm), it’s clear that the Modern Data Stack is in its very *early market* with *innovators* and *early adopters*. Most of the Modern Data Stack tools are brand new; if lucky, you will find version 1.0. We can understand why mid-to-large enterprises have yet to pick up on the latest open-source data tools.Â
...
We see a similar picture when we look at [Google Trends](https://trends.google.com/trends/explore?date=today%205-y&q=%22modern%20data%20stack%22) queries where the term “Modern Data Stack” is seldom searched outside of the US and «Data Twitter»—which is mainly San Francisco if you search in the US only.Â
...
Large enterprises typically ignore the trends until they are unavoidable, which the [Enterprise IT Adoption Cycle](https://enterpriseitadoption.com/) dramatically illustrates. It’s okay, though, as these enterprises predominantly suffer from many legacies and migrations projects already; we will get into that later.


### Pre-Modern Data Stack


While working at larger enterprises, interacting with the business people who need to get the job done, I often heard questions like “[Why can’t we just write a script for this](https://airbyte.com/blog/etl-framework-vs-etl-script)?” and “Does our existing tool (fill in the name) support that already?” I’m generalizing here on purpose, as each enterprise is different and all of them have a department or a couple of people playing with the latest stack.
...
These scripts above were **written in custom code** dedicated to one company—sometimes for one department only.Â


Alternatively, buying an all-in-one enterprise data platform that uses no-code services gets you started with an **out-of-the-box toolbox** for data integration, transformation, orchestration, and visualization right away. These tough decisions usually require several months of evaluation to be sure to invest in the right tool, as prices vary and are not cheap. Your final choices have long-term consequences, as you lock the company into that proprietary tool.


Today you get the full data stack for free, open-sourced on GitHub as illustrated in [Part I](https://www.ssp.sh/blog/open-data-stack-core-tools/). It’s a **game-changer** and is one reason that small companies can outperform large enterprises regularly with strong data-driven strategies and the latest technology from the Modern Data Stack.


**ðï¸ Traditional Data Stack vs. Modern Data Stack**Â


An overview of the data flow pre-MDS with [ETL](https://glossary.airbyte.com/term/etl) and in modern data stack with the shift to centralize data before transforming the data with [ELT](https://glossary.airbyte.com/term/elt).Â
...
Think of the above purple boxes on the left as different teams within the department. None of these were terrible ideas; it’s just the best way there was without investing in expensive SAP or similar solutions.


### Europe is different from the US market


I’m generalizing here, but I’d like to share some high-level observations from working most of my life in European enterprises with 100-100,000 employees.Â
...
With the above-discussed custom, scripts developed and maintained over the years also manifest in many legacy codes. But there is not only legacy code; there is also legacy hardware. Think of the above banks still running the old [IBM Mainframe](https://www.ibm.com/topics/mainframe) dependent on COBOL, Fortran, or some other ancient [programming language](https://glossary.airbyte.com/term/programming-languages) which, according to Credit Suisse, is “still core of our Swiss Banking platform.” You can also check the changes [in salaries between 2021 and 2022](https://survey.stackoverflow.co/2022/#section-top-paying-technologies-change-in-salaries-between-2021-and-2022), and you’ll see a similar picture with COBOL, Delphi, and IBM DB2.


The Seattle Data Guy also rightfully [points out](https://seattledataguy.substack.com/p/realities-of-being-a-data-engineer) that data engineers regularly face migration projects. Some of the types of migrations he mentions are operational system migrations (CRM, ERP), hardware migrations (newer, faster servers), cloud migrations (on-premise to the cloud), analytics migrations (different [data warehouses](https://glossary.airbyte.com/term/data-warehouse) or [BI Tools](https://glossary.airbyte.com/term/business-intelligence-tools)), and data migrations (switch database or schema designs).


## Enterprise Data Platforms


Ultimately, every company needs a data platform for managing its data. So where do mid-to-large sized enterprises start? Not with an open-source Modern Data Stack, as people are used to working with more monolithic approaches with Oracle and Microsoft Servers. Therefore, the easiest and probably the best transition is to use services provided by your cloud provider of choice. These essentially are the MDS stack tools, *operated* by Amazon, Microsoft, and Google—and most of the time, they have renamed the product as well (e.g., [Google Cloud Composer](https://cloud.google.com/composer/docs/concepts/overview) is a managed [Apache Airflow](https://glossary.airbyte.com/term/apache-airflow)).


Think of big vendors such as SAP, Oracle, and Microsoft, which were the main drivers a couple of years ago. Today there are more data platforms to choose from than ever. The big [cloud providers](https://glossary.airbyte.com/term/cloud-provider) are the best known: Amazon Web Services (AWS), Google Cloud, Microsoft Azure, Databricks, and Snowflake, where you find any service you need or find in open-source. Other less known are dedicated enterprise data platforms such as Ascend, Palantir Foundry, Kuwala, Keboola, Nexla, and Rivery, among many more. Also, solutions under [Customer Data Platform](https://glossary.airbyte.com/term/cdp-customer-data-platform) (CDP) are essentially data platforms. Their feature set is vastly different, and each of them has their strengths and weaknesses.


ð **Metadata in Enterprise Data Platforms**


The advantage that enterprise data platforms have is the metadata. As they act end to end, they have much-needed metadata to work in a [declarative](https://glossary.airbyte.com/term/declarative) manner where you define what you want to achieve, and the platform figures out the how. The Modern Data Stack is one tool of many and missing the much-needed metadata to act autonomously. Moreover, the MDS had to work [imperatively](https://glossary.airbyte.com/term/imperative/). But with tools such as [Dagster](https://glossary.airbyte.com/term/dagster) and other tool integrations, this advantage slowly but surely merges more into the MDS, making it more powerful with the declarative approach.


â¹ï¸ **Alternative: The Modern Data Stack Cloud**


Besides the all-in-one data platform, the modern data stack provider offers mainly a cloud option and different deployment options. Besides the prominent cloud provider running the MDS stack on their cloud, each MDS tool has its cloud product, e.g., [Airbyte Cloud](https://cloud.airbyte.io/), [Airflow Cloud (Astronomer)](https://www.astronomer.io/), [Dagster Cloud](http://dagster.io/cloud), etc. These usually offer three different deployment methods:

- On-Prem / Self-managed (Free open-source option)
- Cloud First (Managed open-source version)
- Enterprise / Hybrid (Bring your compute platform, and the control plane of each cloud does not see your code or [PII](https://en.wikipedia.org/wiki/Personal_data) data. Enterprise / Hybrid maximizes flexibility and **security** while offloading the vast majority of the operational burden to the cloud provider)


### No-code vs. code solutions


These big vendors and their enterprise data platforms are essentially big no-code or less-code solutions. It’s important to know the difference between no-code and code solutions. For example, no-code is excellent for **lowering technical barriers** and enabling more users to perform data engineering. But these tools come with significant drawbacks:

- You are building knowledge in a proprietary tool rather than something generic and easily transferable (e.g., coding in [Python](https://glossary.airbyte.com/term/python)).
...
On the other hand, code-first solutions are hard to get started; you need highly skilled software engineers, but you get **sustainable, long-term** **solutions** that are open without vendor lock.
...
In the end, it’s always a **tradeoff** between fully on-premise and hosting all your tools yourself vs. everything managed on SaaS:Â
...
Another consideration is naming. Even though the term Modern Data Stack hasn’t spread much, there is already the dislike of “modern,” which has no tangible value. Alternative names for “Modern Data Stack” that I saw are [new generation open-source data stack](https://blog.devgenius.io/modern-data-stack-demo-5d75dcdfba50) (ngods), DataStack 2.0, and the Boring Data Stack. While starting an open-source [Project](https://github.com/airbytehq/open-data-stack/), I found [Open Data Stack](https://www.ssp.sh/brain/open-data-stack/) to be the perfect nomenclature, emphasizing the **value open-source** tools provide. “Open” also speaks to **open standards**—desperately needed in data engineering.Â
...
 example project we just started (we’ll be adding to it as part of thisÂ
...
Another sign shows [DuckDB](https://glossary.airbyte.com/term/duckdb/) hype; people like the simplicity of it— removing instead of adding to the data stack. Many [use cases](https://glossary.airbyte.com/term/duckdb/#use-cases) are possible, and everyone is looking to simplify things. In the end, simplification also means fewer moving parts and fewer errors.Â
...
Enterprises still have legacy code that will use old programming languages for a while. So these are still slowly adopting [Open Data Stack](https://www.ssp.sh/brain/open-data-stack/) over the next decade. Again, integrating good tools from the data stack doesn’t mean you’ll get a good stack per se. It helps to choose wisely. Ask yourself: “Do we have the capacity and knowledge to own and manage the data stack ourselves?”Â


If you want to try it yourself, follow along with our [open-data-stack](https://github.com/airbytehq/open-data-stack/) project on GitHub, where you will see the core Open Data Stack tools in action.
