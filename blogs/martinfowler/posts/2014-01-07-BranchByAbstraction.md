...
description: "âBranch by Abstractionâ is a technique    for making a large-scale change to a software system in gradual way   that allows you to release the system regularly while the change is   still in-progr"
...
âBranch by Abstractionâ is a technique 1
...
1: 
      ...
      never got a popular name. Paul Hammant introduced the term âBranch by
      Abstractionâ while arguing in favor of [Trunk
      Based Development](http://paulhammant.com/2013/04/05/what-is-trunk-based-development/) (he credits Stacy Curl with originally
      coming up with it). This name seems to have caught on and is now
      the term I most commonly hear.


We begin with a situation where various parts of the software
  system are dependent on a module, library, or framework that we
  wish to replace


We create an abstraction layer that captures the interaction
  ...
  entirely through this abstraction layer.


We gradually move all client code over to use the abstraction
...
We build a new supplier that implements the features required by
  one part of the client code using the same abstraction layer 2. Once
...
2: 
      ...
      [FeatureFlags](https://martinfowler.com/bliki/FeatureFlag.html) to run the new supplier in test
      environments and compare its behavior to the flawed supplier.


We gradually swap out the flawed supplier until all the client
  ...
  layer once we no longer need it for migration.


My description above describes a common case, but there are plenty
...
Despite these variations, there is a common theme. Use an
  ...
  times, so you can continue to use [Continuous Delivery](https://martinfowler.com/delivery.html) while you are
...
[Jez
    Humble describes](http://continuousdelivery.com/2011/05/make-large-scale-changes-incrementally-with-branch-by-abstraction/) how his team used Branch-by-Abstraction to
    ...
    on Rails) for Thoughtworks's Continuous Delivery tool âGoâ.


Paul Hammant provides [more
    details](http://paulhammant.com/blog/branch_by_abstraction.html) of using Branch-by-Abstraction, particularly as an
    alternative to using version-control branching.


Steve Smith [describes a variation](https://www.continuousdeliveryconsulting.com/blog/application-pattern-verify-branch-by-abstraction/) which involves verifying
    that the two implementations return the same results to requests.
