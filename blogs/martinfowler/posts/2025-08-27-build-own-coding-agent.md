...
description: "CLI coding agents are a fundamentally different tool to chatbots or   autocomplete tools - they're agents that can read code, run tests, and update   a codebase. While commercial tools are impressive,"
date: 2025-08-27T00:00:00
tags: ["generative ai"]
url: https://martinfowler.com/articles/build-own-coding-agent.html
slug: build-own-coding-agent
word_count: 2971
---
...
We can run these servers as a local process, so no data sharing, where
      we interact with STDIN/STDOUT to keep things simple and local. [(More details on tools and MCP)](https://martinfowler.com/articles/function-call-LLM.html)
...
**The workflow change:** Now when I mentioned âBedrock is timing outâ
      or âthe model responses are getting truncated,â the agent could directly
...
**The workflow change:** Instead of jumping to solutions, the agent
      ...
      architectural decisions and debugging complex issues. I could ask âWhy is
...
**The workflow change:** This is where everything came together. I
      could now say âThe authentication tests are failing, please fix the issueâ
      and the agent would:
