...
tags: ["evolutionary design", "api design", "refactoring"]
...
Making a change to an interface that impacts all its consumers requires two thinking modes:
      ...
      try to do both at the same time, especially if the change is on a [PublishedInterface](https://martinfowler.com/bliki/PublishedInterface.html)
...
As part of refactoring, we detect that `x` and `y` are a [DataClump](https://martinfowler.com/bliki/DataClump.html)
...
This pattern is particularly useful when practicing [ContinuousDelivery](https://martinfowler.com/bliki/ContinuousDelivery.html) because it
...
- *Refactoring*: when changing a method or function signature, especially when doing a
      [Long Term
      Refactoring](https://martinfowler.com/articles/workflowsOfRefactoring/#long-term) or when changing a [PublishedInterface](https://martinfowler.com/bliki/PublishedInterface.html). A variant implementation of
      this pattern during a refactoring is to implement the old method in terms of the new API and
      use [Inline Method](http://refactoring.com/catalog/inlineMethod.html) to update
...
- *Database refactoring*: this is a key component to
      [evolutionary database design](https://martinfowler.com/articles/evodb.html). Most
...
- *Deployments*: deployment techniques such as canary
      releases  and
      [BlueGreenDeployment](https://martinfowler.com/bliki/BlueGreenDeployment.html) are applications of the parallel change pattern where you
...
- *Remote API evolution*: parallel change can be used to evolve a remote API (e.g.
      ...
      [Postel's Law](http://en.wikipedia.org/wiki/Robustness_principle) is a good
      technique to avoid consumers breaking when the payload is expanded.


During the migrate phase, a [FeatureFlag](https://martinfowler.com/bliki/FeatureFlag.html) can be used to control which version of
...
When implementing [BranchByAbstraction](https://martinfowler.com/bliki/BranchByAbstraction.html), parallel change is a good way to introduce
...
Industrial Logic's
      [
      refactoring album](https://elearning.industriallogic.com/gh/submit?Action=AlbumContentsAction&album=foundations&devLanguage=Java) documents and demonstrates an example of performing a parallel change.
...
This technique was first documented as a refactoring strategy by Joshua Kerievsky in 2006
      ...
      Limited Red Society](http://www.infoq.com/presentations/The-Limited-Red-Society) presented at the Lean Software and Systems Conference in 2010.
