---
title: "Tor for Technologists"
description: "A summary of how Tor works and how you can use it. It also covers The Tor Browser Bundle, Hidden Services, Tails and looks at some of the controversies around Tor."
date: 2015-06-15T00:00:00
tags: ["privacy"]
url: https://martinfowler.com/articles/tor-for-technologists.html
slug: tor-for-technologists
word_count: 3444
---


[Tor](https://torproject.org) is a technology that is cropping up in
    ...
    system that has very specific security requirements.


The Internet is currently a quite hostile place. There are threats
    of all kinds, ranging from [script
    kiddies](http://www.catb.org/jargon/html/S/script-kiddies.html) and drive-by [phishing](https://www.owasp.org/index.php/Phishing)
    attacks to pervasive [dragnet
    surveillance](https://www.eff.org/nsa-spying) by many of the major intelligence services in the
    ...
    Facebook encourages [the use of Tor](https://www.facebook.com/notes/protect-the-graph/making-connections-to-facebook-more-secure/1526085754298237)
...
The goal of Tor is to enable anonymous network traffic. The word
      ...
      who is visiting that server.


Tor works by having thousands of volunteers hosting something
      ...
      information back, using the same process in reverse.


![](tor-for-technologists/sketch.png)


The final relay is usually called an exit relay. The first relay
...
One important point is that you don't have to trust all the Tor
...
Tor is explicitly designed to be a [low-latency network](https://en.wikipedia.org/wiki/Low_latency) - that means it
      is possible to use it for things like Instant Messaging, audio
      conferencing or even video conferencing under good circumstances.


### The Tor client


If you want to use Tor, there are two options: the
        first one is to run the client program, and the second is to
        download and run something called the [Tor Browser
        Bundle](https://www.torproject.org/projects/torbrowser.html.en). If you want to use Tor for other kinds of traffic
        ...
        to tunnel traffic through Tor.


The Tor client program by default will listen to port 9050 using
        a protocol called [SOCKS](https://www.ietf.org/rfc/rfc1928.txt) - this is a
        standard proxy protocol that many applications can be configured to
        use. For example, I use a program called [Gajim](http://gajim.org/) for instant messaging. In this program I
        have configured all my accounts to connect over Tor by specifying
        the correct SOCKS proxy settings.


If you are using command line tools, it is usually possible to
        use a program called *usewithtor* or *torsocks* in order to
        transparently make the program use Tor. There are other ways of
        using Tor as well - [Thunderbird](https://www.mozilla.org/en-US/thunderbird/) can
        ...
        in mail headers in some cases. Fortunately, there exists a plugin called [TorBirdy](https://addons.mozilla.org/En-us/thunderbird/addon/torbirdy/) that automates many of the steps
        and tries to stop such unsafe information leakage from happening.


### The Tor Browser Bundle


By far the easiest way of using Tor is to download the Tor Browser
        ...
        configuring your regular browser to use Tor.


The Tor Browser Bundle comes with several plugins that will improve
...
Tor also has another interesting feature that has gotten a lot of
...
Hidden services are extremely powerful for several reasons. First,
...
Facebook recently made a splash by exposing their servers as
        ...
        is something that in general is good for everyone.


### Bridges and obfuscated protocols


In some regions there is enough censorship going on that using Tor
        ...
        the list of IPs to be blocked.


Because of this situation, the Tor software supports something
        ...
        this problem - but it's an ongoing arms race to beat censorship.


## Anonymity is hard - what doesn't Tor protect against?


Tor is a tool, and just like any tool it has things it does well,
      ...
      correctly, there are things to keep in mind.


The biggest day-to-day problem when it comes to anonymity and
      ...
      the endpoint (such as HTTPS or SMTP with [STARTTLS](https://en.wikipedia.org/wiki/STARTTLS/)). The other way is to use services
      that expose a hidden service - which ensures you get end-to-end
      encryption of all traffic.


Tor is usually classified as a low-latency network. This has a
      ...
      [other kinds of techniques](https://en.wikipedia.org/wiki/Mix_network) that can be
      used, but Tor is not designed for those use cases.


Tor can also not protect you if you make a mistake and expose your
      ...
      dangerous from a privacy-standpoint.


The same caveat is true if you expose your server as a hidden
...
When I talk to technologists about Tor, one of the more common
      ...
      project can always do with more eyeballs.


### Research


All that said, Tor has been broken many times. The main reason for
...
One reason that people are worried about the degree of security of
        ...
        interface and convinced it to send back its real IP address.


And then we have [Operation
        Onymous](https://en.wikipedia.org/wiki/Operation_Onymous). This operation was a collaboration between several
        ...
        propaganda. More info can be found in [this video](https://www.youtube.com/watch?v=pRrFWwA-47U).


Further, looking at the [documents](http://www.theguardian.com/world/interactive/2013/oct/04/tor-stinks-nsa-presentation-document)
        ...
        Tor. I find this to be very encouraging.


## Everyday usage of Tor


Now that you have a better idea of what Tor is, it is time for us
...
The other kind of testing you can do is to make it possible to very
...
As I mentioned before, putting SSH behind a Tor hidden service is a
      ...
      make it impossible to login to the root account remotely.


## Tails


If you happen to be in the kind of situation where Tor is
      ...
      there is a project called [Tails](https://tails.boum.org/), which is a
      Linux distribution that runs from a CD/DVD or USB drive. It is
      preconfigured to *only* send network traffic over Tor and also
...
The interesting thing with Tails is that it's completely amnesiac -
...
Tails is not something you will need in your regular day-job, but
      ...
      to use it before it becomes necessary.


## Conclusion


Tor is a fantastic tool that can do a lot of amazing things. It is
