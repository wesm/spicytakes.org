---
title: "Orm Hate"
description: "While I was at the QCon conference in London a couple of months   ago, it seemed that every talk included some snarky remarks about   Object/Relational mapping (ORM) tools. I guess I should read the  "
...
I think that if you're going to dump on something in the
...
Listening to some critics, you'd think that the best thing for a
    ...
    of Computer Science](http://blogs.tedneward.com/post/the-vietnam-of-computer-science/)1.
...
I've often felt that much of the frustration with ORMs is about
    inflated expectations. Many people treat the relational database âlike a
    crazy aunt who's shut up in an attic and whom nobody wants to talk
    aboutâ3. In this world-view they just want to deal
...
3: 
      I like [this phrase](https://www.amazon.com/gp/product/0321127420/ref=as_li_tl?ie=UTF8&camp=1789&creative=9325&creativeASIN=0321127420&linkCode=as2&tag=martinfowlerc-20) so much I feel
...
There's a consequence to this more limited expectation of what
    ...
    object-relational mapping 4. This doesn't mean you should always
...
4: 
      Indeed in many cases you shouldn't think of building a
      behavior-rich domain model at all, but instead use [Row Data
      Gateways](https://martinfowler.com/eaaCatalog/rowDataGateway.html) that are just the relational tables wrapped in simple
      objects.
...
So am I saying that you should always use an existing ORM rather
    ...
    saying âalwaysâ. One exception that comes to mind is when you're
    ...
    arguments for [CQRS](https://martinfowler.com/bliki/CQRS.html).
...
When it comes to not using relational databases on the disk, there
    ...
    how that worked out. But there is now the new crew of [NoSQL](https://martinfowler.com/nosql.html)
    databases - will these allow us to finesse the ORM quagmire and 
    allow us to shock-and-awe our data storage?


[As you might have gathered](NosqlDistilled.html), I think NoSQL is technology to be
    taken very seriously. If you have an application problem that maps
    well to a NoSQL data model - such as [aggregates](https://martinfowler.com/bliki/AggregateOrientedDatabase.html) or graphs - then you
