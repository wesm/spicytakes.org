...
description: "Self-Testing Code is the name I used inRefactoringto refer to the practice 	of writing comprehensive automated tests in conjunction with the 	functional software. When done well this allows you to inv"
date: 2014-05-01T00:00:00
tags: ["agile", "continuous delivery", "testing", "extreme programming", "programming style", "refactoring"]
url: https://martinfowler.com/bliki/SelfTestingCode.html
slug: SelfTestingCode
word_count: 899
---


Self-Testing Code is the name I used in [Refactoring](https://martinfowler.com/books/refactoring.html) to refer to the practice
...
![](images/selfTestingCode/sketch.png)


I first ran into the thought at an OOPSLA conference listening to
  [âBeddaraâ Dave Thomas ](http://www.davethomas.net) say
  ...
  (and its sister: [TestDrivenDevelopment](https://martinfowler.com/bliki/TestDrivenDevelopment.html)).
...
Self-testing code is a key part of [Continuous Integration](https://martinfowler.com/articles/continuousIntegration.html),
  ...
  Integration, it is also a necessary part of [Continuous Delivery](https://martinfowler.com/delivery.html).
...
But the biggest benefit isn't about merely avoiding production
  ...
  afraid to refactor the system, thus increasing [TechnicalDebt](https://martinfowler.com/bliki/TechnicalDebt.html), and
...
With self-testing code, it's a different picture. Here people are
  ...
  safely, because should you make a mistake (or rather âwhen I make a
  mistakeâ)  the bug detector will go off and you can quickly recover
...
These kinds of benefits are often talked about with respect to
  [TestDrivenDevelopment](https://martinfowler.com/bliki/TestDrivenDevelopment.html) (TDD), but it's useful to separate
...
One important behavior of a team that practices self-testing code
  is the reaction to a production bug. The usual reaction of a team
  using self-testing code is to first write a test that exposes the
  bug, and only then to try to fix it. Often writing this test may
  really be a series of tests that gradually narrow the scope until
  you reach a [UnitTest](https://martinfowler.com/bliki/UnitTest.html) that triggers the bug. This is
  useful as a debugging technique, it's also essential to ensure that
  once the bug is fixed it stays fixed. Usually the team will also use
  this bug as inspiration to look for similar missing tests. The
  attitude should be that any bug isn't just a failure in the code,
  it's equally a failure in the testing screen.


Increasingly these days we're seeing another dimension to
  self-testing, with more emphasis put on monitoring in production. [Continuous Delivery](https://martinfowler.com/delivery.html) allows you to
