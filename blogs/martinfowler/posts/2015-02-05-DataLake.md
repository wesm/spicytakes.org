---
title: "Data Lake"
description: "Data Lake is a term that's appeared in this decade to describe   an important component of the data analytics pipeline in the world ofBig Data. The idea is to   have a single store for all of the raw "
date: 2015-02-05T00:00:00
tags: ["database", "data analytics"]
...
Data Lake is a term that's appeared in this decade to describe
  ...
  [Big Data](https://martinfowler.com/articles/bigData/). The idea is to
...
This is an important step, many data warehouse initiatives didn't
  ...
  [BoundedContexts](https://martinfowler.com/bliki/BoundedContext.html), each with its own data model. In
...
This leads to a common criticism of the data lake - that it's just a
  ...
  title of the New Analytics is âData Scientistâ. Although it's a
...
One of my colleagues illustrated this thinking with a recent
  ...
  âWe were trying to compare our automated predictive models versus
...
So far I've described the data lake as singular point for
  ...
  coined by James Dixon in 2010](https://jamesdixon.wordpress.com/2010/10/14/pentaho-hadoop-and-data-lakes/), when he did that he intended a
  data lake to be used for a single data source, multiple data sources
  would instead form a âwater gardenâ. Despite its original formulation
  the prevalent usage now is to treat a data lake as combining many
  sources. 2


2: 
      In [a later blog post](https://jamesdixon.wordpress.com/2014/09/25/data-lakes-revisited/), Dixon emphasizes the lake versus water
...
It is important that all data put in the lake should have a clear
  ...
  from feeding [Domain Events](https://martinfowler.com/eaaDev/DomainEvent.html)
  into the lake, a natural fit with [Event Sourced](https://martinfowler.com/eaaDev/EventSourcing.html) systems. But it could
  ...
  [ContradictoryObservations](https://martinfowler.com/bliki/ContradictoryObservations.html).
...
The Data Lake's appetite for a deluge of raw data raises awkward
  ...
  [Datensparsamkeit](https://martinfowler.com/bliki/Datensparsamkeit.html) is very much in tension with the data
