---
title: "Unit Test"
description: "Unit testing is often talked about in software development, and   is a term that I've been familiar with during my whole time writing   programs. Like most software development terminology, however, i"
date: 2014-05-05T00:00:00
tags: ["test categories", "extreme programming"]
...
Unit testing is often talked about in software development, and
  ...
  think that it's more tightly defined than it actually is. 1


1: 
      I wrote a little about the historical roots of the name in the entry for [IntegrationTest](https://martinfowler.com/bliki/IntegrationTest.html)


![](images/unitTest/sketch.png)


Although I'd done plenty of unit testing before, my definitive
  exposure was when I started working with Kent Beck and used the [Xunit](https://martinfowler.com/bliki/Xunit.html)
  family of unit testing tools. (Indeed I sometimes think a good term
  for this style of testing might be âxunit testing.â) Unit testing
  also became a signature activity of [ExtremeProgramming](https://martinfowler.com/bliki/ExtremeProgramming.html) (XP), and led
  quickly to [TestDrivenDevelopment](https://martinfowler.com/bliki/TestDrivenDevelopment.html).
...
There were definitional concerns about XP's use
  ...
  berated by a testing expert for misusing the term âunit test.â We
  asked him for his definition and he replied with something like âin
  the first morning of my training course I cover 24 different definitions of
  unit test.â
...
Despite the variations, there are some common elements. Firstly
  ...
  framework 2. Thirdly unit tests are
  expected to be significantly faster than other kinds of tests.


2: 
      I say âthese daysâ because this is certainly something
...
A more important distinction is whether the unit you're testing
    should be sociable or solitary 3. Imagine you're testing
    an order class's price method. The price method needs to invoke
    some functions on the product and customer classes. If you
    like your unit tests to be solitary,
    ...
    Instead you use [TestDoubles](https://martinfowler.com/bliki/TestDouble.html) for the collaborators.
...
![](images/unitTest/isolate.png)


But not all unit testers use solitary unit tests. Indeed when xunit
    testing began in the 90's we made no attempt to go solitary unless
    ...
    to fail. So we felt allowing our tests to be sociable didn't lead
    to problems in practice.


Indeed using sociable unit tests was one of
    the reasons we were criticized for our use of the term âunit
    testingâ. I think that the term âunit testingâ is appropriate because
...
As xunit testing became more popular in the 2000's the notion of solitary tests
    came back, at least for some people. We saw the rise of Mock Objects and frameworks to
    support mocking. Two schools of xunit testing developed, which [I call the classic and mockist styles](https://martinfowler.com/articles/mocksArentStubs.html). One of the
    differences between the two styles is that mockists insist upon solitary unit tests,
    while classicists prefer sociable tests. Today I know and respect xunit testers of
    both styles (personally I've stayed with classic style).


Even a classic tester like myself uses test doubles when there's an
    awkward collaboration. They are invaluable to remove
    [non-determinism
    when talking to remote services](https://martinfowler.com/articles/nonDeterminism.html#RemoteServices). Indeed some classicist xunit testers
...
The common properties of unit tests — small scope, done by the
    ...
    characteristics of [SelfTestingCode](https://martinfowler.com/bliki/SelfTestingCode.html). In this situation
...
When you run unit tests so frequently, you may not run all
    ...
    suite**, since it's what I run whenever I think of compiling -
...
If you are using Continuous Integration you should run a test
    ...
    **commit suite**, to include all the unit tests. It may
    also include a few [BroadStackTests](https://martinfowler.com/bliki/BroadStackTest.html). As a programmer you
...
4: 
      ...
      the commit suite to run, then you should build a [DeploymentPipeline](https://martinfowler.com/bliki/DeploymentPipeline.html)
...
Different people have different standards for the speed of unit tests
    ...
    Heinemeier Hansson](http://david.heinemeierhansson.com/2014/slow-database-test-fallacy.html) is happy with a compile suite that takes a few
    seconds and a commit suite that takes a few minutes. [Gary
    Bernhardt](https://www.destroyallsoftware.com/blog/2014/tdd-straw-men-and-rhetoric) finds that unbearably slow, insisting on a compile suite
    of around 300ms and [Dan Bodart](http://dan.bodar.com/2012/02/28/crazy-fast-build-times-or-when-10-seconds-starts-to-make-you-nervous/) doesn't want his commit suite to be
...
Updated on Oct 24 2014 to include a mention of Field's
    sociable/solitary vocabulary.


Updated on March 9 2017 to make the solitary/sociable terms the primary way of
    describing the distinction and removing the use of the term âcollaborator isolationâ
    (due to confusion with isolating test fixture changes from each other).
