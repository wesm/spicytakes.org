...
description: "We are building an experimental AI co-pilot for product strategy and   generative ideation called “Boba”. Along the way, we’ve learned some useful lessons   on how to build these kinds of applications"
...
Boba is an experimental AI co-pilot for product strategy & generative ideation,
    designed to augment the creative ideation process. It’s an LLM-powered
    application that we are building to learn about:

1. How to design and build generative experiences beyond chat, powered by
      LLMs
2. How to use AI to augment our product and strategy processes and craft


An AI co-pilot refers to an artificial intelligence-powered assistant designed 
...
Boba as an AI co-pilot is designed to augment the early stages of strategy ideation and
    ...
    Boba, we decided to focus on rudimentary versions of the following capabilities:


1. **Research signals and trends:** Search the web for
...
- How do people on Reddit feel about Lululemon's products?â


2. **Creative Matrix: **The creative matrix is a concepting method for
...
- Strategic prompt: “How might we use generative AI to transform wealth
      management?”
- Dimension 1 - Stages of the value chain: Client acquisition,
      financial planning, portfolio construction, investment execution, performance
      monitoring, risk management, reporting and communication
- Dimension 2 - Different personas: For employees, for customers,
      for partners


3. **Scenario building:** Scenario building is a process of
...
4. **Strategy ideation:** Using the Playing to Win strategy
    framework, brainstorm âwhere to playâ and âhow to winâ choices
...
5. **Concept generation:** Based on a strategic prompt, such as a âhow might weâ question, generate 
...
6. **Storyboarding:** Generate visual storyboards based on a simple
...
Boba is a web application that mediates an interaction between a human
...
![](building-boba/mediation.png)


Boba can help with a number of product strategy tasks. We won't
      describe them all here, just enough to give a sense of what Boba does and
      to provide context for the patterns later in the article.


When a user navigates to the Boba application, they see an initial
      screen similar to this


![](building-boba/use-initial-screen.png)


The left panel lists the various product strategy tasks that Boba
...
The above screenshot looks at the scenario design task. This invites
      the user to enter a prompt, such as âShow me the future of retailâ.


![](building-boba/use-future-retail-prompt.png)


The UI offers a number of drop-downs in addition to the prompt, allowing
      the user to suggest time-horizons and the nature of the prediction. Boba
      will then ask the LLM to generate scenarios, using Templated Prompt to enrich the user's prompt
      with additional elements both from general knowledge of the scenario
      building task and from the user's selections in the UI.


Boba receives a Structured Response from the LLM and displays the
      result as set of UI elements for each scenario.


![](building-boba/use-scenario-cards.png)


The user can then take one of these scenarios and hit the explore
      button, bringing up a new panel with a further prompt to have a Contextual Conversation with Boba.


![](building-boba/use-explore-scenario.png)


Boba takes this prompt and enriches it to focus on the context of the
      selected scenario before sending it to the LLM.


Boba uses Select and Carry Context
      ...
      having to worry about supplying the right context for each interaction.


One of the difficulties with using an
      ...
      feature called research signals that uses Embedded External Knowledge
      to combine the LLM with regular search
      facilities. It takes the prompted research query, such as âHow is the
      hotel industry using generative AI today?â, sends an enriched version of
      that query to a search engine, retrieves the suggested articles, sends
      each article to the LLM to summarize.


![](building-boba/use-research-signals.png)


This is an example of how a co-pilot application can handle
      ...
      (as long as the search engine isn't partaking of the wrong mushrooms).


## Some patterns for building generative co-pilot applications


In building Boba, we learnt a lot about different patterns and approaches
      ...
      we've learnt so far while building Boba.
...
The first and simplest pattern is using a string templates for the prompts, also
        ...
        few-shot prompt templates, prompt validation, and more sophisticated composable chains of prompts.


For example, to brainstorm potential future scenarios in Boba, you can
...
![](building-boba/template-initial-screen.png)


The prompt template that powers this generation looks something like
        this:
...
As you can imagine, the LLM’s response will only be as good as the prompt
        ...
        Persona](https://platform.openai.com/docs/guides/gpt-best-practices/tactic-ask-the-model-to-adopt-a-persona),
...
As part of our test-and-learn prompt engineering workflow, we found that
...
We also kept our prompt templates as simple as possible, devoid of
...
Almost any application you build with LLMs will most likely need to parse
...
We were equally surprised by the fact that it could support fairly complex
...
An interesting side effect of describing the JSON response schema was that we
        ...
        prompt at the intersection of each row and column):


![](building-boba/template-matrix.png)


By providing a few-shot prompt that includes a specific example of the output
        schema, we were able to get the LLM to “think” in the right context for each
        idea (the context being the prompt, row and column):
...
We could have alternatively described the schema more succinctly and
        ...
        accurate context for the ideas being generated.


At the time of this writing, OpenAI has released a new feature called
        [Function
        Calling](https://openai.com/blog/function-calling-and-other-api-updates), which
...
One of the first few things you’ll realize when implementing a graphical
        ...
        seeing a response, even if it was a partial one.


Hence, when implementing a co-pilot experience, we highly recommend
...
This allowed us to provide the real-time progress needed to create a smoother
...
![](building-boba/realtime-progress.gif)


However, doing so adds a lot of additional complexity to your application
        ...
        the [Vercel AI SDK](https://github.com/vercel-labs/ai) is a library for building
        edge-ready AI-powered streaming text and chat UIs.
...
One of the biggest limitations of a chat interface is that a user is
        ...
        life in the context of an action or description.


Select and Carry Context allows the user to narrow or broaden the scope of
        ...
        concept. First, the user selects an idea (either explicitly with a checkbox or implicitly by clicking a link):


![](building-boba/select-carry-select.png)


Then, when the user performs an action on the selection, the selected item(s) are carried over as context into the new task,
        for example as scenario subprompts for strategy generation when the user clicks âBrainstorm strategies and questions for this scenarioâ, 
        or as context for a natural language conversation when the user clicks Explore:


![](building-boba/select-carry-carry.png)


Depending on the nature and length of the context
        you wish to establish for a segment of conversation/interaction, implementing
        Select and Carry Context can be anywhere from very easy to very difficult. When
...
Another technique of implementing Select and Carry Context is to do so within
        ...
        generation is collection of selected scenarios:
...
However, when your context outgrows an LLM’s context window, or if you need
        ...
        something similar in Embedded External Knowledge.


If you want to learn more about the effective use of selection and
        context in generative applications, we highly recommend a talk given by
        Linus Lee, of Notion, at the LLMs in Production conference: [“Generative Experiences Beyond Chat”](https://www.youtube.com/watch?v=rd-J3hmycQs).
...
This is a special case of Select and Carry Context.
        ...
        conversation does make the most sense for the user.


In the example below, the user is chatting with Boba about a concept for
        ...
        sports you love...â), and the user has asked Boba to create a user journey for 
        the concept. The response from the LLM is formatted and rendered as Markdown:


![](building-boba/conversation-example.png)


When designing generative co-pilot experiences, we highly recommend
...
While LLMs don’t actually “think”, it’s worth thinking metaphorically
        about a phrase by Andrei Karpathy of OpenAI: [“LLMs ‘think’ in
        tokens.”](https://www.youtube.com/watch?v=bZQun8Y4L2A) What he means by this
...
In some parts of Boba, like strategy and concept generation, we ask the
        LLM to generate a set of questions that expand on the user’s input prompt
        before generating the ideas (strategies and concepts in this case).


![](building-boba/outloud-questions.png)


While we display the questions generated by the LLM, an equally effective
        ...
        description of this pattern can be found in OpenAI’s [GPT Best Practices
        Guide](https://platform.openai.com/docs/guides/gpt-best-practices), in the
        section [Give GPTs time to
        “think”](https://platform.openai.com/docs/guides/gpt-best-practices/strategy-give-gpts-time-to-think)


As a user experience pattern for generative applications, we found it helpful
...
Provide affordances for the user to have a back-and-forth
        interaction with the co-pilot


LLMs are bound to either misunderstand the user’s intent or simply
        ...
        the quality and relevance of the responses generated.


Similarly, we believe that the quality of a generative co-pilot
        ...
        pattern. This can involve several approaches:
...
One example of where we’ve implemented Iterative Response
        in
...
![](building-boba/iterate-hotel.png)
...
![](building-boba/iterate-sdprompt.png)


Another example Iterative Response that we
        ...
        the user in its ranking of next best code suggestions.


We believe that this is one of the most important, albeit
...
Combine LLM with other information sources to access data beyond
        the LLM's training set


As alluded to earlier in this article, oftentimes your generative
...
![](building-boba/embed-hotel.png)


To implement this, we had to “equip” the LLM with Google as an external
...
1. Use a Google SERP API to perform the web search based on the user’s query
          and get the top 10 articles (search results)
2. Read the full content of each article using the Extract API
3. Save the content of each article in short-term memory, specifically an
          ...
          article itself).
4. Generate an embedding of the user’s search query
5. Query the vector store using the embedding of the search query
6. Prompt the LLM to answer the user’s original query in natural language,
          while prefixing the results of the vector store query as context into the LLM
          prompt.


This may sound like a lot of steps, but this is where using a tool like
        ...
        question-answering took only a few lines of code in Boba:
...
The article text contains the entire content of the article, which may not
        ...
        we recommend using an external vector DB like Pinecone or Weaviate.


We also could have further streamlined our workflow by using Langchain’s
        ...
        external tools is to use Open AI’s recently released [Function Calling
        API](https://openai.com/blog/function-calling-and-other-api-updates), which we
        mentioned earlier in this article.


To summarize, we combined two distinct techniques to implement Embedded External Knowledge:

1. Use External Tool: Search and read articles using Google SERP and Extract
          APIs
2. Use External Memory: Short-term memory using an in-memory vector store
          (HNSWLib)
...
So far, we’ve only scratched the surface with the prototype of Boba and what a
      ...
      patterns and practices are yet to be discovered!
