...
Hello! I’ve been thinking about how to explain DNS a bunch [in](https://jvns.ca/blog/2021/02/24/a-little-tool-to-make-dns-queries/) [the](https://jvns.ca/blog/2021/03/31/dnspeep-tool/) [last](https://jvns.ca/blog/2021/11/04/how-do-you-tell-if-a-problem-is-caused-by-dns/) [year](https://jvns.ca/blog/2021/12/06/dns-doesn-t-propagate/).
...
So I built a site where you can do experiments with DNS called [Mess With DNS](https://messwithdns.net). It has examples of experiments you can try, and
...
![](https://jvns.ca/images/messwithdns-recording.gif)
...
![](https://jvns.ca/images/explanation.png)
...
I built this project with [Marie Claire LeBlanc Flanagan](https://marieflanagan.com/), a friend of mine who’s a game designer
...
![](https://jvns.ca/images/sidebar-old.gif)
...
![](https://jvns.ca/images/sidebar-new.gif)


Also, everything in that gif implemented in pure CSS with a `<details>` tag which I
thought was  cool. (here’s [the codepen I learned this from](https://codepen.io/NielsVoogt/pen/XWjPdjO))
...
So we wrote [a short DNS dictionary](https://messwithdns.net/dictionary.html) to try to help with that a bit.
...
![](https://jvns.ca/images/cname-mess.png)
...
used them for ages. I asked [Tom](https://ballingt.com/) what I should use, he suggested [Playwright](https://playwright.dev/), so I used that and it worked great.
...
I’ve put a [snapshot of the backend’s code on GitHub](https://github.com/jvns/mess-with-dns-backend/) if you want to read it.
...
1. start with an empty program that does almost nothing (copied from [How to write a DNS server in Go](https://jameshfisher.com/2017/08/04/golang-dns-server/))
...
[https://github.com/miekg/dns](https://github.com/miekg/dns) which I used previously for the backend of
[https://dns-lookup.jvns.ca/](https://dns-lookup.jvns.ca/).


Here’s what my main [`ServeDNS` function](https://github.com/jvns/mess-with-dns-backend/blob/main/main.go#L284-L304) looks like, with some error handling and logging removed:
...
2. if I get a `HTTPS` query, return any A/AAAA records I have. (I’m doing
...
I did find [this test suite for an authoritative DNS server](https://github.com/dnsimple/dnstest/blob/main/src/dnstest_definitions.erl) which I might spend more time looking through later.
...
So instead I wrote
60ish lines of code using Go channels ([stream.go](https://github.com/jvns/mess-with-dns-backend/blob/7e5ef139a99bfe08683e80bf220b4bf35edb292f/stream.go)).
Basically every time someone opens a websocket, I [create a Go channel](https://github.com/jvns/mess-with-dns-backend/blob/main/main.go#L120)
...
So I googled “asn database download”, and I found this free [IP to ASN database](https://iptoasn.com/) which lists every IP address and who it belongs to.


Then I just needed to [implement a binary search](https://github.com/jvns/mess-with-dns-backend/blob/main/ip2asn.go#L57-L73)
...
So the IP address lookup code [does a reverse DNS lookup and then falls back to the ASN database](https://github.com/jvns/mess-with-dns-backend/blob/main/main.go#L315-L326).


### some notes on picking a database


I started out using [planetscale](https://planetscale.com/) because they have a free tier and I love free tiers.
...
The data volume is attached to a [stateless machine running Postgres](https://fly.io/docs/reference/postgres/) that I can easily upgrade to
...
Like with the [nginx playground](https://jvns.ca/blog/2021/09/24/new-tool--an-nginx-playground/), I
...
So I wrote a simple [login function](https://github.com/jvns/mess-with-dns-backend/blob/7e5ef139a99bfe08683e80bf220b4bf35edb292f/users.go#L108-L117) that:
...
Also the website’s domain ([https://messwithdns.net](https://messwithdns.net))
is hosted on a different domain entirely than the domain where users can set
records (which is [https://messwithdns.com](https://messwithdns.com)). So that means that if someone somehow does set a record on `messwithdns.com` at least it won’t take the site down.
...
wanted. So I set up two static IPv4 addresses: [orange.jvns.ca](http://orange.jvns.ca)
and [purple.jvns.ca](http://purple.jvns.ca). They show a picture of an orange and some
...
[https://messwithdns.net](https://messwithdns.net) and you can report problems [on GitHub](https://github.com/jvns/mess-with-dns-backend/issues).
