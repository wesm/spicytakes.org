...
date: 2022-03-23
url: https://jvns.ca/blog/2022/03/23/a-toy-version-of-tls/
slug: a-toy-version-of-tls
word_count: 2954
---
...
[traceroute](https://jvns.ca/blog/2013/10/31/day-20-scapy-and-traceroute/), [TCP](https://jvns.ca/blog/2014/08/12/what-happens-if-you-write-a-tcp-stack-in-python/) and [DNS](https://jvns.ca/blog/2022/02/01/a-dns-resolver-in-80-lines-of-go/).
What about TLS? Could I implement a toy version of that to learn more about how it works?


I asked on Twitter if this would be hard, got [some encouragement and pointers for where to start](https://twitter.com/Lukasaoz/status/1505593360521777157), so I decided to go for it.


This was really fun and I learned a little more about how involved real
cryptography is – thanks to [cryptopals](https://cryptopals.com/), I already 100% believed that I should not invent my own
...
All of that said, let’s go implement some cryptography! All of my hacky code is [on github](https://github.com/jvns/tiny-tls/). I decided to use Go because I heard that Go has good crypto libraries.
...
reproduce every part. Some googling revealed that it was [The New Illustrated TLS Connection](https://tls13.ulfheim.net).
...
4. The cipher suites/signature algorithms we want to use (which I just copied
...
You can see the rest of the code to generate the [client hello message here](https://github.com/jvns/tiny-tls/blob/cb5a3665c3487ad1f1d5f917ad069c93dd44967e/format.go#L41)
...
for all elliptic curves or just for this specific elliptic curve ([Curve25519](https://en.wikipedia.org/wiki/Curve25519)).
...
parse it to get the server’s public key which is 32 bytes. [Here’s the code though](https://github.com/jvns/tiny-tls/blob/cb5a3665c3487ad1f1d5f917ad069c93dd44967e/format.go#L98-L131).
...
[https://tls13.ulfheim.net](https://tls13.ulfheim.net) provided a bunch of example inputs and outputs and
...
encrypt/decrypt, for More Security (tm).
...
This makes sense to me because I did some of the cryptopals challenges and there’s an attack a bit like this in [cryptopals set 2](https://cryptopals.com/sets/2/challenges/14) (I don’t know if it’s the exact same thing).
...
Next we need to send a “handshake finished” message to the server to verify that everything is done. That code is [here](https://github.com/jvns/tiny-tls/blob/cb5a3665c3487ad1f1d5f917ad069c93dd44967e/crypto.go#L177-L183).
...
the [https://tls13.ulfheim.net](https://tls13.ulfheim.net) example and put it into my unit tests so that I
...
[cryptopals](https://cryptopals.com/) challenges, I really recommend them – you
