---
title: "An attempt at implementing char-rnn with PyTorch"
date: 2020-11-30
url: https://jvns.ca/blog/2020/11/30/implement-char-rnn-in-pytorch/
slug: implement-char-rnn-in-pytorch
word_count: 1636
---
...
2. add another linear transformation after the LSTM, because the LSTM outputs a
vector with size `hidden_size`, and we need a vector that has size
`input_size` so that we can turn it into a character
3. Save the LSTM hidden vector (which is actually 2 vectors) as an instance
...
3. Compute the loss between what the RNN predicted, and the real next letter
...
2. The `output` vector isn’t **yet** a vector of probabilities, so
...
probability.
3. `torch.multinomial(prediction_vector)` takes the vector of probabilities and
