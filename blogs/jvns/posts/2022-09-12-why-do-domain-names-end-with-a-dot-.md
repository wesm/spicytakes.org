...
slug: why-do-domain-names-end-with-a-dot-
word_count: 1788
---


Hello! When I was writing the zine [How DNS Works](https://wizardzines.com/zines/dns/)
...
Also, some DNS tools require domains to have a `"."` at the end: if you try to pass `example.com` to [miekg/dns](https://github.com/miekg/dns), like this, it’ll fail:
...
Even though the zone file format is defined in the official DNS RFC ([RFC 1035](https://www.rfc-editor.org/rfc/rfc1035#section-4.1.1)), you don’t have to
...
1. Because it sees `search lan`, `getaddrinfo` adds a `lan` at the end of `grapefruit` and looks up `grapefruit.lan` instead
...
2. If the domain has an `"."` **inside** it (like `example.com` has a dot in it), then by default search domains aren’t used either. But this can be changed with configuration (see this blog post about [ndots](https://pracucci.com/kubernetes-dns-resolution-ndots-options-and-why-it-may-affect-application-performances.html) that talks about this more)
