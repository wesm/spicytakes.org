...
slug: examples-of-problems-with-integers
word_count: 2787
---


Hello! A few days back we talked about [problems with floating point numbers](https://jvns.ca/blog/2023/01/13/examples-of-floating-point-problems/).
...
So I [asked on Mastodon again](https://social.jvns.ca/@b0rk/109700446576896509) for examples of integer problems and got all kinds of great responses again. Here’s a table of contents.
...
I’m not actually sure if there’s any way to automatically detect integer overflow errors in Go. (though it looks like there’s a [github issue from 2019 with some discussion](https://github.com/golang/go/issues/30613))
...
though, the [wikipedia article has a list](https://en.wikipedia.org/wiki/Signed_number_representations).


### weird thing: the absolute value of -128 is negative


This [Go program](https://go.dev/play/p/iSFxbFAe75M) has a pretty simple `abs()` function that computes the absolute value of an integer:
...
so I’ll mention it: [Rachel by the bay](https://rachelbythebay.com/w/2020/11/26/magic/) has a bunch of great
...
Another integer overflow example: here’s a [search for CVEs involving integer overflows](https://cve.mitre.org/cgi-bin/cvekey.cgi?keyword=integer+overflow).
There are a lot! I’m not a security person, but here’s one random example: this [json parsing library bug](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-24795)
...
There’s a table in [this blog post](https://torstencurdt.com/tech/posts/modulo-of-negative-numbers/) that
...
1. Signed integer overflow in C is **undefined behavior**. I think that’s
...
2. “undefined behaviour” in C means “the compiler is free to do literally whatever it wants after that point”  (see this post [With undefined behaviour, anything is possible](https://raphlinus.github.io/programming/rust/2018/08/17/undefined-behavior.html) by Raph Levine for a lot more)
3. Some compiler optimizations assume that undefined behaviour will never
...
4. So this `if (n + 100 < 0)` check is irrelevant – if that did
...
For Javascript, ESLint has a [no-bitwise check](https://eslint.org/docs/latest/rules/no-bitwise) check), which
