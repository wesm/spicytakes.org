...
slug: new-zine--the-secret-rules-of-the-terminal
word_count: 1179
---
...
[https://wizardzines.com/zines/terminal](https://wizardzines.com/zines/terminal), or get
an [15-pack of all my zines here](https://wizardzines.com/zines/all-the-zines/).
...
exactly *why* these things happen, you’ll probably build an intuition for them.
...
exactly what was *happening* in the terminal to be able to talk about how to
reason about it.
...
When I wrote [How Git Works](https://wizardzines.com/zines/git), I thought I
...
to suggest a [small improvement](https://github.com/fish-shell/fish-shell/issues/10834) to fish!). Identifying exactly which piece of software is causing a weird thing to happen in my terminal still isn’t *easy* but I’m a lot better at it now.
- you can write a shell script to [copy to your clipboard over SSH](https://jvns.ca/til/vim-osc52/)
- how `reset` works under the hood (it does the equivalent of `stty sane; sleep 1; tput reset`) – basically I learned that I don’t ever need to worry about
remembering `stty sane` or `tput reset` and I can just run `reset` instead
...
and more help. I met with [Marie Claire LeBlanc Flanagan](https://marieflanagan.com) every weekday from September to June to work
on this one.


The cover is by Vladimir Kašiković,
Lesley Trites did copy editing,
Simon Tatham (who wrote [PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/)) did technical review, our
Operations Manager Lee did the transcription as well as a million other
things, and [Jesse Luehrs](https://github.com/doy) (who is one of the very few
...
shipped to your house. The only caveat is print orders will ship in **August** – I
