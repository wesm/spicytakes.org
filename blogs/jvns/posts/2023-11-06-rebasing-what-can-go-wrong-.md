...
slug: rebasing-what-can-go-wrong-
word_count: 3165
---
...
So I asked on [Mastodon](https://social.jvns.ca/@b0rk/111342083852635579):
...
1. **rebasing on an ancestor**, like `git rebase -i HEAD^^^^^^^` to squash many
...
I’ve never tried this but I think you need to set `git config rerere.enabled true` and then it’ll automatically help you.
...
Apparently the “since your last **fetch**” is important here – if you run `git fetch` immediately before running `git push --force-with-lease`, the
...
One person shared [their workflow for splitting commits with rebase](https://github.com/kimgr/git-rewrite-guide#split-a-commit).
...
3. Atomic Beautiful Commits – every change is split into the appropriate
