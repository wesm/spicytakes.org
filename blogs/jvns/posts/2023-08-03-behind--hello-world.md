...
date: 2023-08-03
url: https://jvns.ca/blog/2023/08/03/behind--hello-world/
slug: behind--hello-world
word_count: 4274
---
...
In `fish` (my shell), you can see the [path resolution logic here](https://github.com/fish-shell/fish-shell/blob/900a0487443f10caa6539634ca8c49fb6e3ce5ba/src/path.cpp#L31-L45).
...
(if this doesn’t work for you, instead try `strace -o out bash`, and then `grep stat out`. One reader mentioned that their version of libc uses a different
...
faster. Let’s go on a short adventure into inodes. ([this great post “A disk is a bunch of bits” by Dmitry Mazin](https://www.cyberdemon.org/2023/07/19/bunch-of-bits.html) has more details)
...
good sign. We’re not going to go through all of this, but the [ext4 inode struct from the Linux kernel](https://github.com/torvalds/linux/blob/fdf0eaf11452d72945af31804e2a1048ee1b574c/fs/ext4/ext4.h#L769)
...
the `012` means in [man inode](https://man7.org/linux/man-pages/man7/inode.7.html) (scroll down to “The file type and mode”).
...
There are still a bunch of system calls after dynamic linking in the [full strace output](https://gist.github.com/jvns/4254251bea219568df9f43a2efd8d0f5) that I
...
Someone on mastodon [pointed out](https://octodon.social/@lkundrak/110832640058459399) that `ldd` is actually a shell script
...
So it’s calling `__pthread_initialize_minimal`.  I found the [code for that function in glibc](https://github.com/bminor/glibc/blob/a78e5979a92c7985eadad7246740f3874271303f/nptl/nptl-init.c#L100),
though I had to find an older version of glibc because it looks like in more
recent versions [libpthread is no longer a separate library](https://developers.redhat.com/articles/2021/12/17/why-glibc-234-removed-libpthread).
...
if we look at the [cpython source code](https://github.com/python/cpython/blob/400835ea1626c8c6dcd967c7eabe0dad4a923182/Python/fileutils.c#L1955), it does seem to be calling `write()` in some places. So I’m willing to believe that.
