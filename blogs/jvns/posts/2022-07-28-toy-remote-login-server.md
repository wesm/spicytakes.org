...
slug: toy-remote-login-server
word_count: 2753
---


Hello! The other day we talked about [what happened when you press a key in your terminal](https://jvns.ca/blog/2022/07/20/pseudoterminals/).


As a followup, I thought it might be fun to implement a program that’s like a
tiny ssh server, but without the security. You can find it [on github here](https://github.com/jvns/tiny-remote-login/blob/main/server.go), and I’ll explain how it works in this blog post.
...
the information being sent and received in [what happens if you press a key in your terminal](https://jvns.ca/blog/2022/07/20/pseudoterminals/)
...
And you can! We could write a TCP connection handler like this that does exactly that, it’s not a lot of code ([server-notty.go](https://github.com/jvns/tiny-remote-login/blob/main/server-notty.go)).
...
Here’s some Go code to create a pseudoterminal on Linux. This is copied from [github.com/creack/pty](https://github.com/creack/pty/blob/7de28cee0d53510e719c1aeb1850af0fa647c343/pty_linux.go),
...
Here’s the code, it’s pretty short: (again, I just copied it from [creack/pty](https://github.com/creack/pty/blob/7de28cee0d53510e719c1aeb1850af0fa647c343/pty_linux.go#L41-L54))
...
about this kind of thing: [the linux programming interface](https://man7.org/tlpi/), specifically chapter 34 on process groups
and sessions.
...
And that’s it for the server!  You can see all of the Go code here: [server.go](https://github.com/jvns/tiny-remote-login/blob/main/server.go).
...
off called [Entering raw mode](https://viewsourcecode.org/snaptoken/kilo/02.enteringRawMode.html).
...
I figured out how to do this in Go by going to [https://grep.app](https://grep.app) and typing in
...
You can also use the [client.go program](https://github.com/jvns/tiny-remote-login/blob/main/client.go) and run `go run client.go tetris.jvns.ca 7777`.
...
- [toy tls 1.3 implementation](https://jvns.ca/blog/2022/03/23/a-toy-version-of-tls/)
- [toy dns resolver](https://jvns.ca/blog/2022/02/01/a-dns-resolver-in-80-lines-of-go/)
