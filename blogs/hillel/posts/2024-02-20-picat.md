...
2. A set of action functions that represent state transitions. In Picat these functions *must* all be named `action` and take four parameters: a current state, a next state, an action name, and a cost. We’ll see that all below.
...
After writing this I realize I should have instead used a [structure](http://retina.inf.ufsc.br/picat_guide/#x1-530003.4.3) for the action, writing it instead as `$move(T[1])`. I didn’t want to rewrite all of my code though so I’m leaving it like this.
 [return]
...
I used a [Raku script](src/format_path.raku) to visualize it.2 Here’s what we now get:
...
1. If you want just *any* plan, regardless of how long it is, you can call `plan(Start, Plan)` instead.
 [return]
2. The commented version of the formatter was originally up on [my Patreon](https://www.patreon.com/posts/98616250). You can now see it [here](https://gist.github.com/hwayne/94ca3b23078234ca7e803c061b9338f3#file-format_picat-raku).
 [return]
3. “Find the most elements you can remove *without* getting you a valid partition” is a little more convoluted, but you can see it [here](https://gist.github.com/hwayne/94ca3b23078234ca7e803c061b9338f3#file-partition_planner-pi).
 [return]
