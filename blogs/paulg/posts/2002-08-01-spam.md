...
built to exerciseArc. An
improved algorithm is described inBetter
   ...
   (unless (< (+ g b) 5)
     (max .01
          (min .99 (float (/ (min 1 (/ b nbad))
                             (+ (min 1 (/ g ngood))   
                                (min 1 (/ b nbad)))))))))wherewordis the token whose probability we're
calculating,goodandbadare the hash tables
I created in the first step, andngoodandnbadare the number of nonspam and spam messages respectively.I explained this as code to show a couple of important details.
I want to bias the probabilities slightly to avoid false
positives, and by trial and error I've found that a good
way to do it is to double all the numbers ingood.
...
the mail is spam.  Ifprobsis a list of the fifteen individual probabilities, you
calculate thecombinedprobability thus:(let ((prod (apply #'* probs)))
...
the developer of the filter.  How manypointsshould an
...
Words that occur disproportionatelyrarelyin spam (like "though" or "tonight" or "apparently")
...
Indeed, mostantispam techniquesso far have been like pesticides that
...
http://www.27meg.com/foobecause that is about as much sales pitch as content-based
...
may not decrease the amount of spam that spammerssend,they can certainly help filters to decrease the amount of  
...
business.  That wordbusinessis an important one to
...
worth their while to send spam.The reason the spammers use the kinds ofsales
pitchesthat they do is to increase response rates.
...
whorespondsto a spam.  This person is either
...
spammers to tune spams to get through them.Appendix: Examples of FilteringHereis an example of a spam that arrived while I was writing
...
qves
7c5
7c266675The words are a mix of stuff from the headers and from the
...
probabilities we have to look atthisactually quite
...
very            0.14758544
valuable        0.82347786This time the evidence is a mix of good and bad.  A word like  
...
word "madam"neveroccurs in my legitimate email, and
...
before the results start to be thrown off.For comparison,hereis an example of that rare bird, a spam that
...
prices     0.9359873
managed    0.06451222
difficult  0.071706355There are a couple pieces of good news here.  First, this mail
...
recognized as  a spam term.Finally,hereis an innocent email.
...
spot          0.1654587
us-ascii      0.16804294
what          0.19212411Most of the words here indicate the mail is an innocent one.
...
defining feature of spam (in fact, itsraison d'etre)
...
promoting a porn site.I propose we define spam asunsolicited automated email.
...
a domain name is about to expire (at least, if they are theactual 
registrarfor it).  But when Verisign sends me
...
test a new Lisp dialect calledArcthat is 
...
of X is probably the last place in the world you'd want to live.Thanksto Sarah Harlin for reading drafts of this; Daniel Giffin (who is 
...
and Sam Steingold for advice about statistics.You'll find this essay and 14 others inHackers & Painters.More Info:

[Better Bayesian Filtering](http://paulgraham.com/better.html)

[Filters that Fight Back](https://paulgraham.com/ffb.html)
...
[Probability](https://paulgraham.com/naivebayes.html)

[Spam is Different](https://paulgraham.com/spamdiff.html)

[Filters vs. Blacklists](https://paulgraham.com/falsepositives.html)

[Trust Metrics](http://www.levien.com/free/tmetric-HOWTO.html)

[Filtering Research](https://paulgraham.com/bayeslinks.html)

[Microsoft Patent](https://paulgraham.com/msftpatent.html)
...
[CRM114 gets 99.87%](https://paulgraham.com/wsy.html)
