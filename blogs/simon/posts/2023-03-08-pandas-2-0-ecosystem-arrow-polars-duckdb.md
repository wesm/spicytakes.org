...
In the recent update,Â [Pandas 2.0](https://datapythonista.me/blog/pandas-20-and-the-arrow-revolution-part-i/)Â has adopted Apache Arrow as a backend. This article explores why this is the most significant change in the update and how Arrow has become a fundamental part of many recently released frameworks and hot topics in data engineering.


That makes it an excellent time to reflect on what Pandas is and why itâs successful.

ð¿ Interesting to see the ecosystem evolving aroundÂ Apache Arrow
...
, Cubeâs Cache LayerÂ
[Cube Store](https://cube.dev/blog/introducing-cubestore)
,Â
[Dremio](https://dremio.com/)
. But why? Letâs discuss that.

## What is Pandas


But letâs start with what Pandas is? Pandas is an library toÂ **process data in memory**Â with a feature-rich Python API. Compared to many tools, Pandas isÂ *the*Â python library to work with smaller data and sets the standard for wrangling data in memory.
...
To understand what is new and better with the latest version, letâs briefly discuss how Pandas works.


Before doing anything with Pandas, the general idea is to load data in-memoryÂ **into a Pandas DataFrame**, usually using functions like read_csv, read_sql, read_parquet,Â [etc](https://pandas.pydata.org/docs/reference/frame.html). When loading data into memory, it must decide how it will be stored in memory.
...
[Python](https://glossary.airbyte.com/term/python/)Â is versatile and can represent mostly anything, but PythonÂ [data structures](https://docs.python.org/3/tutorial/datastructures.html)Â (lists, dictionaries, tuples, etc.) are very slow and canât be used. So the data representation is not Python and isÂ **not standard**, and implementation needs to happen via Python extensions, usually implemented in C (also in C++, Rust, and others).


For many years, the main extension to represent arrays and perform operations on them quickly has beenÂ [NumPy](https://numpy.org/). Which is also what Pandas was initially built on.
...
That brings us to version 2.0. Letâs look at the key improvements: what are the highlights, and why do we have them?
...
All of these are achieved through using Apache Arrow as the backend. By moving from NumPy (C++) to Apache Arrow as a backend (especially In Pandas 1.5 and 2.0 added Arrow support for all data types), Arrow gives it better storage and speed. A significant milestone was implementing aÂ [string data type based on Arrow](https://github.com/pandas-dev/pandas/pull/35259)Â that started in 2020.
...
[Apache Arrow](https://glossary.airbyte.com/term/apache-arrow/)Â sets the open standard to exchange in a heterogeneous data pipeline, which needs to read and share data among different steps.
...
One of the biggest problems of Pandas was data interchange, particularly moving large tabular datasets from one processâs memory space to anotherâs.


The significant achievement here isÂ **zero-copy data access**, mapping complex tables to memory to make accessing one terabyte of data on disk as fast and easy as one megabyte.


[

](https://www.ssp.sh/blog/pandas-2-0-ecosystem-arrow-polars-duckdb/images/arrow-ecosystem.jpg)Arrow ecosystem and standardization fromÂ [Overview Apache Arrow](https://arrow.apache.org/overview/).
...
Arrow is a program-independent format. Similar toÂ [Data Lake File Formats](https://glossary.airbyte.com/term/data-lake-table-format)Â but less obvious, as it does not have a file extension attached and only exists in memory.
...
When you have a data pipeline that loads some data from your data lake, transforms it, computes some analytics, and then exports and automatically creates aÂ [Data Asset](https://glossary.airbyte.com/term/data-asset/)Â that can be used for analytics, you usually have different tools, abstractions, or even teams in more prominent companies. If you do not need to persist the data set at each step, thatâs a tremendous boost in performance and cost, as you do not need to store so much data immediately.

Open Standard
This becomes even more powerful as Arrow becomesÂ
*the*
Â open standard for in-memory data representation.
...
People who are used to SQL usually prefer something other than the API of Pandas. Too bloat and too many “gotchas” is what peopleÂ [say](https://www.linkedin.com/posts/maxhumber_im-going-to-get-roasted-for-saying-this-activity-6965384696654442497-Lgr4?utm_source=share&utm_medium=member_desktop).
...
The data ecosystem isÂ [growing](https://mad.firstmarkcap.com/)Â every day, giving us plenty of alternatives to choose from. Letâs look at these alternatives and try to understand why they were created and when they should be used.
...
*Star history of GitHub repositories (LinkÂ or withÂaligned timeline)*
...
SomeÂ [say](https://news.ycombinator.com/item?id=34968769)Â Polars has a less confusing API and better ergonomics, especially from SQL. Polars is more performant out of the box but less stable and mature. It is growing the fastest of all mentioned in this chapter.
...
Â object (or the other way round). There is little to convert since, internally, both libraries implement the exact data representation specification, the Apache Arrow specification. The bulk of the data, GBs or TBs, must not be serialized into another format. This allows data sharing easily, even with “big data”.
...
Daniel BeachÂ [says](https://seattledataguy.substack.com/p/why-is-polars-all-the-rage)Â that Polars is the more accessible version of Spark and easier to understand than Pandas.

Polars and Arrow
Polars internal data representation is Apache Arrow, and now with Pandas 2.0, itâs also one of the possible internal representations for Pandas DataFrames. To be entirely correct, itâs Arrow2, a Rust structure to specific (there is another Arrow implementation in Rust, the officialÂ
...
[DuckDB](https://glossary.airbyte.com/term/duckdb/)Â would be for SQL lovers. Sure, itâs a database format, but in case you didnât know, DuckDB is also super strong as a zero-copy layer approach. For example, you can use DuckDB as a thin SQL wrapper on top of your S3 files across your Data Lake. DuckDB will do a great job of providing you with fast analytical queries.


DuckDBÂ [can efficiently run SQL queries directly on Pandas DataFrames](https://duckdb.org/2021/05/14/sql-on-pandas.html). If you want to use SQL and have a fast interface, use DuckDB.


DuckDB can alsoÂ [query Arrow](https://duckdb.org/2021/12/03/duck-arrow.html)Â datasets directly and stream query results back to Arrow. This streaming allows users to query Arrow data using DuckDBâs SQL Interface and API while taking advantage of DuckDBâs parallel vectorized execution engine without requiring extra data copying. Additionally, this integration takes full advantage of Arrowâs predicate and filter pushdown while scanning datasets.


More (how it works, benchmarks, example projects) about DuckDB on theÂ [Data Glossary](https://glossary.airbyte.com/term/duckdb/).


### What about Dask?


Many people may think ofÂ [Dask](https://github.com/dask/dask)Â as helping with Pandas performance and scalability. It optimizes speed by parallelizing large datasets into pieces and working with them in separate threads or processes or rescuing Pandas from the RAM limit.
...
[Koalas](https://github.com/databricks/koalas)Â is another one, built on top of Spark, re-implemented 70%+ of the Pandas API by Databricks, and nowÂ [officially included in PySpark since Apache Spark 3.2](https://issues.apache.org/jira/browse/SPARK-34849). Interesting to see that Spark has a similar growth rate of GitHub stars as Pandas, especially asÂ [Spark DataFrames](https://spark.apache.org/docs/latest/sql-programming-guide.html)Â are very similar to Pandas, supporting a rich Python API and SQL. But instead of stored locally, itâs persisted inside the Spark cluster.


[Vaex](https://github.com/vaexio/vaex)Â is a high-performance Python library for lazyÂ **out-of-core DataFrames**Â (similar to Pandas) to visualize and explore big tabular datasets.
